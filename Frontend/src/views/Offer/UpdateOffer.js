...
function UpdateOffer() {
  ...
  const getLead = async () => {
    try {
      ...
      if (lead.status === 200) setLeadData(lead.data.leadData);
    } catch (error) {
        ...
        <form onSubmit={handleSubmit}>
          <Grid container spacing={2} paddingTop={2} style={{ marginBottom: '20px' }}>
            <Grid item style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              ...
              <Button size="small" variant="outlined" style={{ color: 'black', borderColor: 'black', margin: '5px' }}>
                {' '}
                Draft
              </Button>
            </Grid>
                ...
                <Select id="client" name="client" size="small" fullWidth value={formData.client} onChange={handleChange}>
                  {clientData.length > 0 ? (
                    clientData.map((client, index) => {
                      return (
                        <MenuItem value={client._id} key={index + 1}>
                          {client.Name}
                        </MenuItem>
                      );
                    })
          ...
          <Grid spacing={3} style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', margin: '10px' }}>
            <Grid item xs={12}>
              <Button onClick={handleAddItem} style={{ color: 'black', border: '1px ddotted black' }} variant="text">
          ...
          <Grid container spacing={2} style={{ padding: '10px' }}>
            <Grid item xs={12} md={6} lg={6}>
              <Button type="submit" style={{ border: '1px ddotted black' }} variant="contained">
                <AiOutlinePlus style={{ margin: '3px' }} /> Save
              </Button>
            </Grid>
            <Grid item xs={12} md={6} lg={6}>
